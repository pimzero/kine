...
#define DL_LAZY_SYM(Lib, Sym) "DL_LAZY_sym_" Lib "." Sym
#define DL_LAZY_LIB(Lib) "DL_LAZY_lib_" Lib

#define DL_LAZY_DECL_COMMON(Decl, Symname)                                \
	__asm__ __volatile__(".comm " Symname ", " _xstr(                 \
		__SIZEOF_POINTER__) ", " _xstr(__SIZEOF_POINTER__) "\n"); \
	extern Decl __asm__(Symname);

#define DL_LAZY(Lib, X)                                                        \
	({                                                                     \
		DL_LAZY_DECL_COMMON(void *_lib, DL_LAZY_LIB(Lib));             \
		DL_LAZY_DECL_COMMON(__typeof__(X) *_sym_##X,                   \
				    DL_LAZY_SYM(Lib, #X));                     \
		if (!_sym_##X) {                                               \
			if (!_lib)                                             \
				_lib = _dlopen_or_die(Lib,                     \
